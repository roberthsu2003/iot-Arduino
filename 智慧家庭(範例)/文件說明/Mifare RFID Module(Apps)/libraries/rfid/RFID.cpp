...
/*
 * Function¡GMFRC522_Auth
 * Description¡GVerify card password
 * Input values¡GauthMode--Passowrd authentication mode
             0x60 = Verify A key
             0x61 = Verify B key
             BlockAddr--Block address
             Sectorkey--Sector's password
             serNum--Card serial number, 4 bytes
 * Output value¡GSuccessful return MI_OK
 */
...
/*
 * Function¡GMFRC522_SelectTag
 * Description¡GElection card, read memory capacity
 * Input values¡GserNum--incoming card serial number
 * Output value¡GSuccessful return of card capacity
 */
 unsigned char RFID::SelectTag(unsigned char *serNum)
 {
    ...
    status = MFRC522ToCard(PCD_TRANSCEIVE, buff, 9, buff, &recvBits);
